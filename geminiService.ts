...
export async function analyzeCV(cvText: string, jdText: string): Promise<AnalysisResult> {
  // Initializing with the environment variable API_KEY
...
export async function rewriteCV(cvText: string, jdText: string, analysis?: AnalysisResult): Promise<RewriteResult> {
  ...
  const prompt = `
    ...
    Output must include a JSON object with:
    - improvedText: The full rewritten CV.
    - keyChanges: A list of main improvements.
    - scoreImprovement: Expected increase in ATS score.
    - keywordReport: { integrated: string[], notIntegrated: { keyword: string, reason: string }[] }
    - formatFixes: List of formatting enhancements.
  `;
